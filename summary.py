...
_SYSTEM = (
    ...
    "произошло в чате.\n"
    "— ШУТКИ И ПРОЗВИЩА: устойчивые фразы, которые завели и повторяют "
    "участники, и кто их придумал. Свои собственные обороты сюда не пиши.\n"
    "— ФАКТЫ О ТЕБЕ: что участники про тебя узнали и как к тебе относятся, "
    "чтобы ты оставался собой между разговорами. Не как ты формулируешь: "
    "любимые словечки сюда не идут.\n"
    ...
    # Заметки уезжают в промпт следующего ответа. Записанная присказка
    # оттуда возвращается в реплику, попадает в новые заметки — и бот
    # неделю говорит про «механизм накаливания». Так и было.
    "— Свои собственные присказки и повторы не записывай ни в один раздел: "
    "ты читаешь эти заметки перед ответом и начнёшь повторять записанное.\n"
